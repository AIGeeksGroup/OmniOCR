...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("lora_fine_tune_rolmocr_shrunk_fixed_log.txt"), logging.StreamHandler()]
)
...
output_dir = "./RolmOCR_lora_tuned_shrunk_fixed"
...
def check_dataset_integrity(dataset_path):
    ...
    for cls in classes:
        ...
        if len(images) < 3:  # 至少需要3张图片以确保每个分割至少1张
...
def load_model_and_processor(model_path):
    ...
    try:
        ...
        logger.info("模型已加载（bfloat16，启用梯度检查点）")
    except Exception as e:
    ...
    logger.info("LoRA适配器已应用，仅微调q_proj和v_proj参数")
...
# 训练函数
def train_model(model, processor, train_loader, val_loader, splits, class_names, num_epochs=1, learning_rate=5e-6, accum_steps=8):
    ...
    class_weights = torch.tensor([1.0] * 10).to(accelerator.device)  # 类别数改为10
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx in range(total_batches):
            try:
                ...
                if len(images) != 10 or len(labels) != 10:  # 采样10个样本
                    logger.warning(f"批次 {batch_idx} 样本数量不足: {len(images)}，跳过")
                    continue
                
                prompt = f"请返回藏语数字的类别编号（0-9）。"  # 更新提示
                messages = [
                    {
                        "role": "user",
                        "content": [
                ...
                try:
                    if batch_idx == 0 and epoch == 0:
                        with profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA], record_shapes=True) as prof:
                            with record_function("model_inference"):
                                ...
                                if not image_inputs or len(image_inputs) != 10:  # 检查10个样本
                                ...
                                if logits.shape[0] != 10 or logits.shape[1] != len(class_names):  # 检查10个样本
                    ...
                    logger.info(f"显存分配详情:\n{torch.cuda.memory_summary()}")
                    logger.error("处理10个样本显存不足，尝试减小image_size或使用float16")
                    return False
                except Exception as e:
        ...
        if val_accuracy > best_val_accuracy:
            ...
            logger.info(f"保存最佳LoRA模型，验证准确率: {best_val_accuracy:.4f}")
...
def evaluate_model(model, processor, dataset_path, splits, split_name="test", image_size=128):
    model.eval()
    class_names = [str(i) for i in range(10)]  # 类别为0-9
    predictions = []
    true_labels = []
...
def main():
    ...
    training_successful = False
    try:
        training_successful = train_model(model, processor, train_loader, val_loader, splits, class_names, num_epochs=1, learning_rate=5e-6, accum_steps=8)
    except Exception as e:
    ...
    if training_successful:
        try:
            torch.cuda.empty_cache()
            torch.cuda.reset_peak_memory_stats()
            logger.info(f"加载最佳LoRA模型从 {output_dir}")
